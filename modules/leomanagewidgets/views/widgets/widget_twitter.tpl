  ...
  if ( $('[id*=leo-twitter{/literal}{$twidget_id|escape:'html':'UTF-8'}{literal}]').length ) {
   $('#leo-twitter{/literal}{$twidget_id|escape:'html':'UTF-8'}{literal} iframe').each( function(){
    ...
    if ( ibody.find( '.timeline-TweetList li.timeline-TweetList-tweet' ).length ) {
		ibody.find( '.timeline-TweetList-tweet .timeline-Tweet-text' ).css( 'color', '{/literal}{$text_color|escape:'html':'UTF-8'}{literal}' );
		ibody.find( '.TweetAuthor-screenName.Identity-screenName' ).css( 'color', '{/literal}{$mail_color|escape:'html':'UTF-8'}{literal}' );
		ibody.find( '.TweetAuthor-name.Identity-name.customisable-highlight' ).css( 'color', '{/literal}{$name_color|escape:'html':'UTF-8'}{literal}' );
		if(show_scroll == 1){
